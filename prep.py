...
def review_iter():
    ...
    for fpath in review_file_iter():
        ...
        if not (product_info.get('Price') or 'Unavailable').startswith('$'):
            logging.warning("Skippig product {} due to price '{}'".format(
...
def reviews_to_x_y(enc, reviews):
    # type: (Encoder, List[Dict[str, Union[int, str, float]]]) -> Tuple[ndarray, ndarray, ndarray]
    ...
    """
    x = numpy.array(list(enc.transform([r['Content'] for r in reviews], fixed_length=100)),
                    dtype='int32')
    y_title = numpy.array(list(enc.transform([r['Title'] for r in reviews], fixed_length=10)),
                          dtype='int32')
...
def prep(cfg: ModelConfig):
    ...
    encoder = Encoder(vocab_size=cfg.vocab_size, silent=False)
    ...
    x, y_title, y_overall = reviews_to_x_y(encoder, reviews)
